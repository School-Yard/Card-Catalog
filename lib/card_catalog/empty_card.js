 ...
 * is set.
 */
Card.prototype.route = function route(req, res, callback) {
  var self = this;

  if(this._static) {
    this._static.serve(req, res, function(err) {
      if(err) self.match(req, res, callback);
    });
  }
  else {
    self.match(req, res, callback);
  }
};
 ...
 */
Card.prototype.match = function match(req, res, callback) {
  var self = this,
  ...
  Matcher(path, routes, function(err, obj) {
    ...
    obj.route.call(self, req, res, callback, obj.params);
  });
};
