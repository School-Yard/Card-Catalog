 ...
 *
 * Matches a slug to a key in the cache object. The
 * object must have a published == true state to be returned.
 * Returns the cache object or false
 */
Category.prototype.match = function(slug) {
  var match;

  if( slug in this.cache) {
    match = this.cache[slug];
    return match.published === true ? match : false;
  } else {
    return false;
  }
};
